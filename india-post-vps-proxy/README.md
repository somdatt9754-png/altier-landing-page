...
Run this service on the existing Hostinger VPS so India Post sees the VPS public/static IP instead of a Supabase Edge Function egress IP.
...
Put the service behind the VPS's existing HTTPS/reverse-proxy setup and expose only `/health` and `/proxy`. Do not expose port 8080 directly if the VPS already has a reverse proxy/firewall.
...
After the VPS HTTPS endpoint is live, configure these secrets for the `india-post-booking` Edge Function:
